...
def currency_symbol(rules):
    return CURRENCY_SYMBOLS.get(rules.get("currency", "PHP"), rules.get("currency", "") + " ")
...
def split_regular_overtime(daily_hours_list, weekly_threshold, daily_threshold):
    ...
    more overtime. This is a default assumption (common daily/weekly OT
...
def cmd_learn(args):
    ...
    for eid, entries in punches.items():
        ...
        print(f"  {name} ({eid}): rate={sym}{rate:.2f}/hr  reg={regular:.2f}h  "
...
def cmd_compute(args):
    ...
    for row in report_rows:
        print(f"{row['name']:<20} reg={row['regular_hours']:>6}h  "
              f"ot={row['overtime_hours']:>5}h  rate={sym}{row['hourly_rate']:.2f}  "
              f"gross={sym}{row['gross_pay']:.2f}")
